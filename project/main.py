...
def process_single_paper(arxiv_id_or_url: str, category: str | None = None) -> str:
    ...
    # reader/summary agents now return the notes BODY only; wrap it with the
    # Obsidian front-matter + title header for the single-paper printout.
    if selected_category != "general" and selected_category in reader_agents:
        ...
        paper["skill_category"] = selected_category
        return ReaderAgent._format_final_notes(paper, body)

    body = summary_agent.summarize(paper)
    paper["skill_category"] = "general"
    return SummaryAgent._format_note(paper, body)
