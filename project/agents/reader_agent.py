...
class ReaderAgent:
    ...
    def read_paper(
        ...
        """
        Execute the full reading pipeline and return the Markdown notes body
        (no YAML front-matter / title wrapper). The caller is responsible for
        wrapping — e.g. ``daily_polymer._format_note`` or
        ``ReaderAgent._format_final_notes``.
        """
        ...
        if self._should_read_appendix(detailed_notes) and parsed_paper.has_appendix:
            ...
            # Drop the APPENDIX_NEEDED directive before feeding notes to pass 3
            detailed_notes = self._strip_appendix_marker(detailed_notes)
            ...
            return detailed_notes.strip()

        return self._strip_appendix_marker(detailed_notes)
    ...
    @staticmethod
    def _strip_appendix_marker(notes: str) -> str:
        """Remove the trailing APPENDIX_NEEDED: YES/NO directive line.

        The pass-2 prompt asks the model to append this directive so we can
        decide whether to read the appendix; it must not leak into the saved
        notes / HTML report.
        """
        return re.sub(
            r"\n*APPENDIX_NEEDED:\s*(?:YES|NO)[^\n]*",
            "",
            notes,
            flags=re.IGNORECASE,
        ).strip()
