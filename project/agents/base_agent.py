...
def create_chain(
    ...
    """Simple  prompt | llm  chain for tool-less single-shot calls.

    Lighter than ``create_agent`` (no LangGraph graph). Invoke with
    ``{"input": "<user text>"}``; returns an ``AIMessage`` whose
    ``.content`` is the model response. Use this for classify / summarize
    style calls that don't need tool-calling.
    """
